...
# train['text'] = 'anchor:' + train['anchor'] + '</s>' + 'target:' + train['target'] + '</s>' + 'context:' + train['context_text']
...
for fold in range(5):
    ...
    optimizer = Ranger21(
        optimizer_parameters, lr=CONFIG.LR, num_epochs=5,
        num_batches_per_epoch = len(train_dl)//2,
    ...
    for epoch in range(CONFIG.EPOCHS):
        ...
        if CONFIG.SAVE_BEST_ONLY:
            if best_score < score:
                best_score = score
                model_name = f'best_roberta_base_model_fold_{fold}'
