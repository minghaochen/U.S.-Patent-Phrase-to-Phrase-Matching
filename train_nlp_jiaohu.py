...
class Model(nn.Module):
    ...
    def loss(self, outputs, targets):
        ...
        return nn.MSELoss(reduction="mean")(outputs.reshape(-1,), targets)
    ...
    def forward(self, input_ids, attention_mask,
        ...
        #
        # mean_pooling_embeddings = torch.cat([anchor_emb, target_emb, anchor_emb-target_emb,
        #                                      torch.mul(anchor_emb, target_emb)], 1)

        # anchor_emb = anchor_emb + context_emb
        # target_emb = target_emb + context_emb

        # # inner product
        anchor_emb = nn.functional.normalize(anchor_emb, dim=-1)
        target_emb = nn.functional.normalize(target_emb, dim=-1)
        logits = torch.sum(anchor_emb*target_emb, dim=1)
...
val_anchors_3 = anchors[3*val_sz:]
val_anchors = [val_anchors_0,val_anchors_1,val_anchors_2,val_anchors_3]
